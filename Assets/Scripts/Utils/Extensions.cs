...
public static class Extensions
{
	...
	public static Vector3 CoordsToVector(this Coordinates coords, IMapConfig config)
	{
		var offset = config.GetOffset(coords.Layer);
		return new Vector3(
		coords.X * config.TileOffsetX + offset.x,
		-coords.Y * config.TileOffsetY + offset.y,
		-coords.Layer
		);
	}

	public static bool Equals(this Coordinates a, Coordinates b)
	{
		return a.X == b.X && a.Y == b.Y && a.Layer == b.Layer;
	}
	public static int PairsInCollection(this IEnumerable<Tile> tiles)
	{
		return tiles.Where(b => !string.IsNullOrEmpty(b.ID))
					.Select(b => b.ID)
					.GroupBy(id => id)
